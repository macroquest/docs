...
``````<font color="blue">``\#event`</font>` CombineError "\#_\#There was no place to put that\#_\#"````` `\| Sub DoCombine``` /varsetEndingVar 1``    
````` /nomodkey /notifyTradeskillWndCombineButton leftmouseup\``````
